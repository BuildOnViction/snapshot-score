 ...
 */
 ...
 async function uniqueUsers() {
     db.Transaction.aggregate([
         ...
         voters.forEach(async (voter) => {
             if (voter.count > 0) {
                 let history = await db.Transaction.find({
 ...
 async function votedVolumnAndDuration() {
     let voters = await db.Voters.find({});
     voters.forEach(async (voter) => {
         let history = JSON.parse(voter.history);
         history = _.sortBy(history, (event) => {
             return new Date(event.createdAt).valueOf()
         })
         let stakedDuration: any = [];
         let lastEvent;
 
         for (let index = 0; index < history.length; index++) {
             let currentEvent = history[index];
             currentEvent.capacity = new BigNumber(currentEvent.capacity);
             ...
             if (!lastEvent) {
                 ...
                 let lastStaking = { ...stakedDuration[stakedDuration.length - 1] };
                 stakedDuration[stakedDuration.length - 1].endDate = currentEvent.createdAt;
                 lastStaking.capacity = new BigNumber(lastStaking.capacity);
             ...
             } else if (currentEvent.event == "Unvote") {
                 let lastStaking = { ...stakedDuration[stakedDuration.length - 1] };
                 stakedDuration[stakedDuration.length - 1].endDate = currentEvent.createdAt;
                 lastStaking.capacity = new BigNumber(lastStaking.capacity);
                 ...
                 if (capacity[0] == "-") {
                     console.log("history minus --- ", capacity)
                     console.log("currentEvent ", currentEvent.tx)
                 }
         ...
         await db.Voters.findOneAndUpdate({ _id: voter._id }, {
 ...
 async function doReport() {
     ...
     var data = [
         ['Voter', 'Avg Capacity', 'Total Capacity', 'Total Duration', 'Active', 'Is TomoPool owner'],
     ];
     ...
     voters.forEach((voter) => {
         ...
         let avgCapacity = new BigNumber(0)
         ...
         voter.staked.forEach((duration) => {
             ...
             avgCapacity = avgCapacity.plus(
                 new BigNumber(duration.capacity)
             );
         ...
         data.push([
             ...
             avgCapacity.div(
                ...
                new BigNumber(voter.staked.length)
            )
            .toFixed(2),
            avgCapacity.div(
                new BigNumber(1000000000000000000)
     ...
     XLSX.writeFile(wb, "voters_filtered.xlsx");
